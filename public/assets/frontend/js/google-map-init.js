var WSUS_googlemap_init_obj = {};
var WSUS_GEOCODE_ERROR = "Error";
// Google map Styles
var WSUS_googlemap_styles = {
    'default': [],
...
function googlemap_init(dom_obj, coords) {
    ...
    try {
        ...
        var id = dom_obj.id;
        WSUS_googlemap_init_obj[id] = {};
        WSUS_googlemap_init_obj[id].dom = dom_obj;
        WSUS_googlemap_init_obj[id].point = coords.point;
        WSUS_googlemap_init_obj[id].description = coords.description;
        WSUS_googlemap_init_obj[id].title = coords.title;
        WSUS_googlemap_init_obj[id].opt = {
            zoom: coords.zoom,
            center: latlng,
            scrollwheel: false,
            scaleControl: true,
            ...
            styles: WSUS_googlemap_styles[coords.style ? coords.style : 'default'],
        ...
        custom_map.geocode(coords.latlng!=='' ? {'latLng': latlng} : {"address": coords.address}, function(results, status) {
            if (status == google.maps.GeocoderStatus.OK) {
                WSUS_googlemap_init_obj[id].address = results[0].geometry.location;
                googlemap_create(id);
            } else {
                alert(WSUS_GEOCODE_ERROR + ' ' + status);
            }
        });

        jQuery(window).resize(function() {
            if (WSUS_googlemap_init_obj[id].map) {
                WSUS_googlemap_init_obj[id].map.setCenter(WSUS_googlemap_init_obj[id].address);
            }
        });
...
function googlemap_create(id) {
    "use strict";
    if (!WSUS_googlemap_init_obj[id].address) {
        return false;
    }
    WSUS_googlemap_init_obj[id].map = new google.maps.Map(WSUS_googlemap_init_obj[id].dom, WSUS_googlemap_init_obj[id].opt);
    WSUS_googlemap_init_obj[id].map.setCenter(WSUS_googlemap_init_obj[id].address);
    var markerInit = {
        map: WSUS_googlemap_init_obj[id].map,
        position: WSUS_googlemap_init_obj[id].address   //WSUS_googlemap_init_obj[id].map.getCenter()
    };
    if (WSUS_googlemap_init_obj[id].point) {
        markerInit.icon = WSUS_googlemap_init_obj[id].point;
    }
    if (WSUS_googlemap_init_obj[id].title) {
        markerInit.title = WSUS_googlemap_init_obj[id].title;
    }
    var marker = new google.maps.Marker(markerInit);
    var infowindow = new google.maps.InfoWindow({
        content: WSUS_googlemap_init_obj[id].description
    });
    google.maps.event.addListener(marker, "click", function() {
        infowindow.open(WSUS_googlemap_init_obj[id].map, marker);
    });
}


function googlemap_refresh(){
    for(id in WSUS_googlemap_init_obj){
        googlemap_create(id);
    }
}
